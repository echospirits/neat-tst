...
- Status-filter visual polish: remove enclosing borders, raised shadows, and persistent selection outlines from the top filter boxes; retain colored arrows, filter icons, Selected labels, hover feedback, and keyboard-only focus rings. CSS-only; no schema/configuration changes.
